            ...
            if request.method == 'POST':
                ...
                for blocked in phone_blocked_chars:
                    if phone and blocked.lower() in phone.lower():
                        ...
                        # ブロックされた場合は安全な値に置換
                        safe_phone = "ブロックされた入力値"
                        return render_template('admin/edit_user.html', user=user, error_phone=safe_phone)
...
def admin_reviews():
    ...
    if user_id == '1':
        try:
            ...
            if updated_comment:
                # scriptタグやイベントハンドラをブロック
                blocked_strings = ['<', '>', 'script', 'javascript:', 'on', '-', '&']
                for blocked in blocked_strings:
                    if blocked.lower() in updated_comment.lower():
            ...
            if request.method == 'POST':
                ...
                if submitted_token:
                    ...
                    if token_count > 0:
                        ...
                        blocked_strings = ['<', '>', 'script', 'javascript:', 'on']
                        for blocked in blocked_strings:
                            if blocked.lower() in comment.lower():
